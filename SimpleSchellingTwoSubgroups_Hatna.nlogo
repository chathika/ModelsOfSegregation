...
globals [
  ...
  global-max-threshold;; storing the variance of threshold as a global to avoid excessive computations
...
turtles-own [
  ...
  threshold ;; the minimum fraction of friends that make an aent happy (utility of 1)
...
to setup
  ...
  set threshold-group-B threshold-group-A
  ask turtles [ifelse one-of [true false] [set threshold threshold-group-A][set threshold threshold-group-B]]
  ...
  set global-max-threshold max [threshold] of turtles
...
to color-by-threshold
  ask turtles [
    set color red + threshold * white
  ]
end
...
;  )
  ...
  ifelse  utility-here < threshold [
    report utility-here / threshold
  ] [
    report 1
  ]
  report utility-here
